...
public class PlayerHealth : MonoBehaviour
{
	...
	{
		hasBeenCaught = true;
		if(collectibleManager.ReturnCollected() > 0)
		{
			int chips = collectibleManager.ReturnCollected();
			double chipsLost = chips * 0.75;
			collectibleManager.Deduct(Mathf.FloorToInt((float)chipsLost));
		}
